...
def send_jar(host, jar):
    print("Jar: " + jar["class"])
    print("Sending the proper jar to the workstation " + host["host"])
...
def upload(wait):
    ...
    for host in hosts:
        ...
        try:
            ssh_testing = paramiko.SSHClient()
            ssh_testing.load_system_host_keys()
            ssh_testing.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh_testing.connect(host["host"], username=host["user"], password=host["password"], timeout=4)
            ssh_testing.exec_command("echo \"Hello!\"")
            ssh_testing.close()
        except paramiko.ssh_exception.SSHException or socket.timeout:
            print("Host " + host["host"] + " is " + CRED + "down" + CEND + ".")
            hosts.remove(host)
            ssh_testing.close()
            continue

        print("Host " + host["host"] + " is " + CGREEN + "up" + CEND + ".")
    ...
    for json_host in jars_hosts:
        try:
            ...
            print("Host " + json_host["host"]["host"] + " is " + CRED + "down" + CEND + ".")
            exit(1)
            continue
