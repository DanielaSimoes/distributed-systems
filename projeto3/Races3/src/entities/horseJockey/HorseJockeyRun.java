...
public class HorseJockeyRun {
    ...
    public static void main(String [] args) {   
        ...
        interfaces.stable.IHorseJockey si = null;
        interfaces.controlCentre.IHorseJockey cci = null;
        interfaces.racingTrack.IHorseJockey rti = null;
        interfaces.paddock.IHorseJockey pi = null;
        interfaces.log.IHorseJockey li = null;
        
        try
        { 
            Registry registry = LocateRegistry.getRegistry (rmiRegHostName, rmiRegPortNumb);
            li = (interfaces.log.IHorseJockey) registry.lookup (RegistryConfigs.logNameEntry);
        }
        ...
        { 
            Registry registry = LocateRegistry.getRegistry (rmiRegHostName, rmiRegPortNumb);
            si = (interfaces.stable.IHorseJockey) registry.lookup (RegistryConfigs.stableNameEntry);
        }
        catch (RemoteException e)
        { 
        ...
        { 
            Registry registry = LocateRegistry.getRegistry (rmiRegHostName, rmiRegPortNumb);
            cci = (interfaces.controlCentre.IHorseJockey) registry.lookup (RegistryConfigs.controlCentreNameEntry);
        }
        catch (RemoteException e)
        { 
        ...
        { 
            Registry registry = LocateRegistry.getRegistry (rmiRegHostName, rmiRegPortNumb);
            rti = (interfaces.racingTrack.IHorseJockey) registry.lookup (RegistryConfigs.racingTrackNameEntry);
        }
        catch (RemoteException e)
        { 
        ...
        { 
            Registry registry = LocateRegistry.getRegistry (rmiRegHostName, rmiRegPortNumb);
            pi = (interfaces.paddock.IHorseJockey) registry.lookup (RegistryConfigs.paddockNameEntry);
        }
        catch (RemoteException e)
        { 
        ...
        for (int i = 0; i < N_HORSEJOKEY; i++){
            horses.add(new HorseJockey(si, cci, pi, rti, (int) (Math.random() * (Constants.HORSE_MAX_STEP_SIZE - 1)) + 1, i));
        }
